...
def inspect(root: Path) -> dict[str, object]:
    ...
    files = sorted(source.rglob("*Controller.java")) if source.is_dir() else []
    ...
    for path in files:
        ...
        for match in METHOD_MAPPING.finditer(text):
            ...
            endpoint = {"method": method, "path": normalize(base, child), "file": str(path.relative_to(root))}
            endpoints.append(endpoint)
        for method, child in REQUEST_METHOD.findall(text):
            endpoints.append({"method": method, "path": normalize(base, child), "file": str(path.relative_to(root))})
    ...
    return {"root": str(root), "controllers_scanned": len(files), "endpoints": endpoints, "errors": errors, "warnings": warnings, "findings": findings}
...
def main() -> int:
    ...
    if args.json:
        ...
        print(f"Summary: {len(report['endpoints'])} endpoint(s), {report['errors']} error(s), {report['warnings']} warning(s)")
