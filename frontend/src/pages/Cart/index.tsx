...
const Cart = () => {
  ...
  const activeItems = localCart.filter(
  ...
  const handleCheckout = () => {
    if (!termsAccepted) {
      alert(
        "Vui lòng đồng ý với Điều khoản & Điều kiện mua hàng tại PinkPhone.",
      );
      return;
    }
      ...
      <div className="min-h-screen bg-[#f7f7fb] py-12">
        <div className="mx-auto w-full max-w-[1400px] px-4 sm:px-6 lg:px-8">
          <div className="rounded-[32px] border border-gray-200 bg-white p-8 text-center shadow-sm sm:p-12">
            ...
            <h2 className="mb-2 text-[24px] font-bold text-[#222222]">
              Giỏ hàng của bạn đang trống
            </h2>
            <p className="mx-auto mb-8 max-w-2xl text-[14px] leading-6 text-[#777777]">
              Hãy chọn một chiếc điện thoại phù hợp và quay lại đây để hoàn tất
        ...
        <div className="mb-8 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              {localCart.length} sản phẩm đang chờ thanh toán
            </p>
          </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.45fr_0.8fr]">
          <div className="space-y-4">
            <div className="rounded-[24px] border border-gray-200 bg-white p-4 shadow-sm sm:p-5">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <label className="flex items-center gap-3 text-[14px] font-medium text-gray-800">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setLocalCart((prev) =>
                        prev.map((item) =>
                          item.outOfStock ? item : { ...item, active: checked },
                        ),
                      );
                    }}
                ...
                <button
                  onClick={() => {
                    const toDeleteIds = localCart
                      .filter((item) => item.active && !item.outOfStock)
                      .map((item) => item.id);
                    setLocalCart((prev) =>
                      prev.filter((item) => !toDeleteIds.includes(item.id)),
                    );
            ...
            <div className="space-y-3">
              {localCart.map((item) => (
                <div
                  key={item.id}
                  className="rounded-[24px] border border-gray-200 bg-white p-4 shadow-sm sm:p-5"
                >
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className="text-[15px] font-bold leading-snug text-[#222222]">
                        {item.name}
                      </h3>
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {cartPromos.map((promo, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="flex min-h-[84px] items-center gap-3 rounded-[14px] border border-[#FDE6EC] bg-white p-4"
                    >
                      ...
                      <span className="text-[12px] font-semibold leading-snug text-[#222222]">
                        {promo.title}
                      </span>
                    </div>
                  );
                ...
                {crossSellProducts.map((prod) => (
                  ...
                  >
                    ...
                    <div className="mb-3 flex items-baseline justify-center gap-1.5">
                      <span className="text-[13px] font-extrabold text-[#E91E63]">
                        {prod.price}
                      </span>
          ...
          <div className="space-y-4 xl:sticky xl:top-24">
            <div className="rounded-[24px] border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <a
                href="#vouchers"
                className="mt-3 block text-[13px] font-semibold text-[#E91E63] hover:underline"
              >
            ...
            <div className="rounded-[24px] border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 space-y-3">
                <label
                  className={`flex items-start gap-3 rounded-[12px] border p-4 transition ${shippingMethod === "delivery" ? "border-[#E91E63] bg-[#FFF8FA]" : "border-[#EEEEEE] bg-white"}`}
                >
                  <input
                    type="radio"
                    name="shipping"
                  ...
                  <div className="flex flex-col">
                    <span className="flex items-center gap-1.5 text-[13px] font-bold text-[#222222]">
                      <Truck className="h-4 w-4 text-[#E91E63]" /> Giao hàng tận
                      nơi
                    </span>
                    <span className="mt-1 text-[11px] font-medium text-[#777777]">
                      Dự kiến nhận 1-3 ngày
                    </span>
                  </div>
                </label>

                <label
                  className={`flex items-start gap-3 rounded-[12px] border p-4 transition ${shippingMethod === "pickup" ? "border-[#E91E63] bg-[#FFF8FA]" : "border-[#EEEEEE] bg-white"}`}
                >
                  <input
                    type="radio"
                    name="shipping"
                  ...
                  <div className="flex flex-col">
                    <span className="flex items-center gap-1.5 text-[13px] font-bold text-[#222222]">
                      <Store className="h-4 w-4 text-[#E91E63]" /> Nhận tại cửa
                      hàng
                    </span>
                    <span className="mt-1 text-[11px] font-medium text-[#777777]">
                      Miễn phí, có hàng sau 2h
                    </span>
                  </div>
                </label>
              </div>
            ...
            <div className="rounded-[24px] border border-[#E91E63]/20 bg-gradient-to-br from-[#FFF8FA] to-white p-5 shadow-sm">
              ...
              <div className="mt-4 flex items-baseline justify-between">
                ...
                <div className="text-right">
                  <div className="text-[22px] font-extrabold leading-none text-[#E91E63]">
                    {formatPrice(total)}
                  </div>
                  <span className="mt-1 block text-[10px] font-medium text-[#777777]">
                    (Đã bao gồm VAT)
                  </span>
                </div>
              </div>
              ...
              <label className="mt-4 flex cursor-pointer items-start gap-2.5 text-[11px] font-medium leading-relaxed text-[#777777]">
                ...
                <span>
                  ...
                  <a
                    href="#terms"
                    className="font-bold text-[#E91E63] underline"
                  >
